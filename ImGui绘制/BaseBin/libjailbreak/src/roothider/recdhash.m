...
extern bool code_directory_calculate_page_hash(CS_DecodedBlob *codeDirBlob, MachO *macho, int slot, uint8_t *pageHashOut);
...
int ensure_randomized_cdhash_for_slice(const char* inputPath, uint64_t offset, void* cdhashOut)
{
	...
	do
	{
		...
		*rd2 = jbinfo(jbrand);
		...
		if(!code_directory_calculate_page_hash(bestCDBlob, macho, 0, pageHash)) {
